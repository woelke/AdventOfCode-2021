...
data Content = Val Integer
             | Op Integer [Packet] deriving Show
...
parseSubPackets :: Integer -> BinString -> (Content, BinString)
...
parseContent binStr = let (typeId, remainder) = parseTypeId binStr
                       in dispatch typeId remainder
  where parseTypeId ys = (fromJust $ binStr2Integer $ take 3 ys, drop 3 ys)
        dispatch typeId ys
          | typeId == 4 = parseLiteralValue ys
          | otherwise = parseSubPackets typeId ys

parseSubPackets typeId binStr = dispatchFunByLengthTypeId $ tail binStr
  where dispatchFunByLengthTypeId
        ...
        parseByLength xs = let len = fromIntegral $ fromJust $ binStr2Integer $ take 15 xs
                            ...
                            in (Op typeId packets, notParsedRemainder)
